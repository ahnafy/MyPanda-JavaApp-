...
public class StateListDrawable extends AbsObjectWrapper<android.graphics.drawable.StateListDrawable> {
    ...
    public static Drawable build(Object prev, HashMap<String, Object> d, boolean designer, Object tag) {
        ...
        Drawable enabled = (Drawable) DynamicBuilder.build(prev, (HashMap) d.get("enabledDrawable"), designer, tag);
        Drawable pressed = (Drawable) DynamicBuilder.build(prev, (HashMap) d.get("pressedDrawable"), designer, tag);
        int[] iArr = new int[]{State_Disabled};
        sld.addState(iArr, (Drawable) DynamicBuilder.build(prev, (HashMap) d.get("disabledDrawable"), designer, tag));
        sld.addState(new int[]{State_Pressed}, pressed);
        sld.addState(new int[0], enabled);
        return sld;
    }
}
